...
export default function SpaceXMassDistribution() {
  ...
  const starshipFuelMass = Math.max(
    starshipMaxFuelMass - payload * 0.5,
    starshipMaxFuelMass * 0.7
  );
    ...
    <Card className='w-full max-w-2xl bg-white-300 rounded-md bg-clip-padding backdrop-filter backdrop-blur-md bg-opacity-90 border border-gray-100'>
      <CardHeader className='border-b border-white/10'>
        ...
        <CardDescription className='text-indigo-200/70'>
          Adjust the payload mass to see how it affects the distribution of mass
          in the Starship system heading to Mars.
        </CardDescription>
      </CardHeader>
        ...
        <div className='backdrop-blur-md p-4 rounded-lg border border-white/20'>
          ...
          <ul className='space-y-1 text-indigo-100/80 text-sm'>
            <li>
              ...
              {totalMass.toFixed(0)} tons
            </li>
            <li>
              <span className='font-medium'>Fuel Percentage:</span>{' '}
              {(
                ((boosterFuelMass + starshipFuelMass) / totalMass) *
                100
              ).toFixed(1)}
              %
            </li>
            <li>
              <span className='font-medium'>Dry Mass Percentage:</span>{' '}
              {(((boosterDryMass + starshipDryMass) / totalMass) * 100).toFixed(
                1
              )}
              %
            </li>
            <li>
              <span className='font-medium'>Payload Percentage:</span>{' '}
              {((payload / totalMass) * 100).toFixed(1)}%
            </li>
          </ul>
        </div>
      </CardContent>
